...
class {{ name | classize }} {
	...
	{% if not additional_properties %}
		const additionalKeys = Object.keys(dict)
			.filter(key => !{{ name | classize }}.Properties.includes(key));
		if (additionalKeys.length > 0) {
			return new Error(`{{name}} is not valid: it contains${additionalKeys.length > 1 ? '' : ' an'} additional key${additionalKeys.length > 1 ? 's' : ''}: ${badKeys.join(', ')}.`);
		}
